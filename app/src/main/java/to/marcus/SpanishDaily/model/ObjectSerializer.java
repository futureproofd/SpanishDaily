...
public class ObjectSerializer{
    ...
    //For new installations (first run)
    public ArrayList<Word> loadDefaultWords(String wordDataSet) throws IOException, JSONException{
        ArrayList<Word> words = new ArrayList<>();
        //parse JSON using JSONTokener
        JSONArray array = (JSONArray) new JSONTokener(wordDataSet).nextValue();
        //Build the array of words from JSONObjects (call the custom Word constructor)
        for (int i = 0; i < array.length(); i++){
            words.add(new Word(array.getJSONObject(i)));
        }
